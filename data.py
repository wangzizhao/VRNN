...
def get_stock_data(stock_idx_name, time, n_train, n_test):
	...
	# convert to float 32
	data = np.array([data_pt[1:] for data_pt in data_list[1:]], dtype=np.float32)		

	obs_train = np.zeros((n_train, time, 6))	# 6 refers to Open, High, Low, Close, Adj Close, Volume
	obs_test  = np.zeros((n_test,  time, 6))
